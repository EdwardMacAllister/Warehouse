...
# Define states for employee actions
class EmployeeStates(StatesGroup):
    waiting_for_employee_info = State()
    waiting_for_delete_info = State()
...
async def cmd_add_employee(message: types.Message):
    ...
    await EmployeeStates.waiting_for_employee_info.set()
...
async def process_add_employee_info(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        with SessionLocal() as db:
            employee = create_employee(name=name, position=position)
...
async def cmd_delete_employee(message: types.Message):
    ...
    await EmployeeStates.waiting_for_delete_info.set()
